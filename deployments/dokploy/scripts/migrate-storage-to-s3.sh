...
################################################################################
# Plane Storage Migration Script (Dokploy Compatible)
################################################################################
...
DOCKER_NETWORK="plane-backend-network" # Updated for Dokploy
...
if ! docker ps --format '{{.Names}}' | grep -q "^${CONTAINER_NAME}$"; then
    ...
    if [ "${start_container}" == "yes" ]; then
        # Assuming the user can start it via Dokploy or docker-compose
        print_info "Please start the container manually via Dokploy or docker-compose and re-run this script."
        exit 1
    else
        exit 1
    fi
